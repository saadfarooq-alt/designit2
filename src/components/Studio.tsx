...
export function Studio({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="flex flex-col h-[100dvh] w-full bg-[#F9F9FB] text-slate-900 overflow-hidden select-none touch-none font-sans" onClick={() => setContextMenu(null)}>
      ...
      <header className="h-16 flex items-center justify-between px-2 lg:px-8 bg-white/80 backdrop-blur-md border-b border-slate-200 shrink-0 z-[100]">
        <div className="flex items-center gap-2">
          <button 
            onClick={() => setIsSidebarOpen(true)} 
            className="lg:hidden bg-yellow-400 text-black px-3 py-2 rounded-xl text-[9px] font-black uppercase shadow-sm"
          >
            Trace
          </button>
          <div className="hidden sm:flex flex-col">
            <span className="text-[10px] font-black uppercase tracking-[0.1em]">DesignIt <span className="text-yellow-500">.</span></span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          <button 
            ...
            className="px-2 sm:px-4 py-2 bg-emerald-50 text-emerald-600 rounded-full text-[8px] sm:text-[9px] font-black uppercase border border-emerald-100"
          >
            {window.innerWidth < 640 ? "Rst" : "Reset"}
          </button>

          <button 
            onClick={() => setGlobalShowDots(!globalShowDots)} 
            className={`px-2 sm:px-4 py-2 rounded-full text-[8px] sm:text-[9px] font-black uppercase border transition-all ${globalShowDots ? 'bg-yellow-50 border-yellow-200 text-yellow-700' : 'bg-white text-slate-400'}`}
          >
          ...
          <button 
            ...
            className={`px-2 sm:px-4 py-2 rounded-full text-[8px] sm:text-[9px] font-black uppercase border transition-all ${isLocked ? 'bg-sky-500 border-sky-600 text-white' : 'bg-white border-sky-100 text-sky-500'}`}
          >
            {isLocked ? (window.innerWidth < 640 ? "Lck" : "Locked") : (window.innerWidth < 640 ? "Unl" : "Unlock")}
          </button>
        </div>
      </header>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        `}>
          <div className="p-6 shrink-0 bg-white border-b lg:border-0 border-slate-100">
            ...
            <div className="grid grid-cols-2 gap-2 mb-3">
               ...
               }} className="bg-yellow-50 text-yellow-700 py-4 rounded-xl text-[10px] font-black uppercase border border-yellow-200">Sample</button>
               ...
               }} disabled={sourceDots.length === 0} className="bg-slate-900 text-yellow-400 py-4 rounded-xl text-[10px] font-black uppercase shadow-lg disabled:opacity-20">Add Design</button>
            </div>
            
            <button onClick={() => fileInputRef.current?.click()} className="w-full bg-white border border-yellow-200 text-yellow-600 py-4 rounded-xl text-[10px] font-black uppercase mb-2">Upload Reference</button>
          ...
          <div className="flex-1 p-4 lg:p-6 overflow-hidden bg-slate-50 lg:bg-white">
            <div className="h-full bg-slate-100 rounded-[2rem] border border-slate-200 relative overflow-hidden flex items-center justify-center">
              <svg viewBox={`0 0 ${imgDims.width} ${imgDims.height}`} className="w-full h-full max-h-[60vh] lg:max-h-none p-4 lg:p-6">
                ...
                {candidates.map(c => (
                  <path key={c.id} d={c.d} fill={c.selected ? "rgba(250, 204, 21, 0.4)" : "transparent"} stroke={c.selected ? "#eab308" : "#cbd5e1"} strokeWidth={4} className="cursor-pointer" onClick={(e) => { e.stopPropagation(); setCandidates(prev => prev.map(x => x.id === c.id ? {...x, selected: !x.selected} : x)); }} />
                ))}
        ...
        <main className="flex-1 bg-[#F9F9FB] relative overflow-hidden">
          ...
          {/* TOOLBAR */}
          <div className="absolute left-2 lg:left-8 top-1/2 -translate-y-1/2 flex flex-col gap-2 p-2 bg-white/70 backdrop-blur-xl border border-white rounded-[2rem] shadow-xl z-50">
