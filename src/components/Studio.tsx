...
export function Studio({ onBack }: { onBack: () => void }) {
  ...
  const sweepErase = (x: number, y: number) => {
    ...
    setWorkspaceShapes(prev => prev.map(s => ({ ...s, dots: s.dots.filter(d => Math.hypot(s.position.x + (d.x * s.scale) - x, s.position.y + (d.y * s.scale) - y) > ERASE_RADIUS) })));
  };
          ...
          <div className="p-6 shrink-0 bg-white">
            ...
            <div className="grid grid-cols-2 gap-2 mb-3">
               ...
               <button id="add-btn" onClick={() => {
                  ...
                  setWorkspaceShapes(prev => [...prev, { id: `s-${Date.now()}`, img: selectedImage!, dots: [...sourceDots], dims: { ...imgDims }, position: { x: 100, y: 100 }, scale: fs, showDots: true }]);
