          ...
          <div className="flex-1 p-4 overflow-hidden">
            <div className="h-full bg-slate-100 rounded-3xl overflow-hidden flex items-center justify-center relative">
              <svg id="trace-svg-container" viewBox={`0 0 ${imgDims.width} ${imgDims.height}`} className="w-full h-full p-4">
                ...
                {candidates.map((c, idx) => (<path key={c.id} id={idx === 0 ? "path-0-0" : c.id} d={c.d} fill={c.selected ? "rgba(250, 204, 21, 0.4)" : "transparent"} stroke={c.selected ? "#eab308" : "#cbd5e1"} strokeWidth={4} className="cursor-pointer" onClick={() => setCandidates(prev => prev.map(x => x.id === c.id ? {...x, selected: !x.selected} : x))} />))}
              </svg>
            </div>
          </div>
        </aside>
